...
def schedule_asset_map():
    d = json.loads(get("https://api.github.com/repos/sportsdataverse/sportsdataverse-data/releases/tags/espn_cfb_schedules"))
...
def main():
    ...
    for season in range(2004, 2027):
        ...
        if sch is None:
            rows.append({"season": season, "betting_rows": len(bet), "schedule_available": False})
            continue
        ...
        teams = pd.unique(pd.concat([m.loc[matched, "home_team"], m.loc[matched, "away_team"]]))
        rows.append({
            "season": season,
            ...
            "spread_pct_of_scored": round(100 * (spread & scored).sum() / max(scored.sum(), 1), 1),
    ...
    print()
    print("totals:", {c: (int(cov[c].sum()) if cov[c].dtype != object else "-") for c in ["betting_rows", "matched_to_schedule", "with_final_scores", "with_closing_spread", "two_sided_closing_ml"]})
    ...
    allx = pd.concat([pd.read_parquet(RAW / f"betting_{s}.parquet") for s in range(2004, 2026)], ignore_index=True)
    ct = allx.groupby(["odds_source", "game_spread_available"], observed=True).size().rename("rows").reset_index()
    print()
    print("odds_source x game_spread_available:")
    print(ct.to_string(index=False))
    equiv = bool(((allx.odds_source == "default") == (~allx.game_spread_available)).all())
